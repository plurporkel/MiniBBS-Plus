...
if (isset($_POST['post'])) {
    ...
    if (error::valid()) {
        ...
        if ($editing) {
            ...
            $db->q('INSERT INTO pages (url, page_title, content, markup) VALUES (?, ?, ?, ?)', $page_data['url'], $page_data['title'], $page_data['content'], $page_data['markup']);
...
if (isset($_POST['preview']) && !empty($page_data['content'])) {
    $preview = $page_data['content'];
    if ($page_data['markup']) {
        $preview = parser::parse($preview);
    }
    echo '<h3 id="preview">Preview</h3><div class="body standalone"> <h2>' . htmlspecialchars($page_data['title']) . '</h2>' . $preview . '</div>';
}
?>
...
<form action="" method="post">
    ...
    <div class="row">
        <input type="radio" name="syntax" value="HTML" class="inline" <?php echo !$page_data['markup'] ? 'checked' : '' ?>> HTML
        <input type="radio" name="syntax" value="markup" class="inline" <?php echo $page_data['markup'] ? 'checked' : '' ?>> <a href="<?php echo DIR ?>markup_syntax">Markup syntax</a>
    </div>
    <div class="row">
