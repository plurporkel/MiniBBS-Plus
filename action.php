...
switch ($_GET['action']) {
    ...
    case 'unhide_image':
        ...
        if (isset($_POST['confirm'])) {
            ...
            if ($ignored_phrases === false) {
                error::fatal('No ignore list found for your user.');
            }
            $ignored_phrases = str_replace($id, '', $ignored_phrases);
        ...
        if (isset($_POST['confirm'])) {
            ...
            switch ($revision->type) {
                case 'page':
                    ...
                    $previous_content = $previous->fetchColumn();
                    if ($previous_content === false) {
                        error::fatal('No page found with that ID.');
                    }
                ...
                case 'topic':
                    ...
                    $previous_content = $previous->fetchColumn();
                    if ($previous_content === false) {
                        error::fatal('No topic found with that ID.');
                    }
                ...
                case 'reply':
                    ...
                    $previous_content = $previous->fetchColumn();
                    if ($previous_content === false) {
                        error::fatal('No reply found with that ID.');
                    }
            ...
            $db->q('INSERT INTO revisions (type, foreign_key, text) VALUES (?, ?, ?)', $revision->type, $revision->foreign_key, $previous_content);
    ...
    case 'dismiss_pm':
        ...
        if (isset($_POST['confirm'])) {
            ...
            $redirect_to = isset($new_parent) ? 'private_message/' . $new_parent . ($new_pm != $new_parent ? '#reply_' . $new_pm : '') : 'private_messages';
    ...
    case 'delete_pm':
        ...
        if (isset($_POST['confirm'])) {
            $res = $db->q('SELECT parent FROM private_messages WHERE id = ?', $id);
    ...
    case 'delete_image':
        ...
        $type = isset($_GET['topic']) ? 'reply' : 'topic';
        $redirect_to = $type == 'reply' ? 'topic/' . (int) $_GET['topic'] . '#reply_' . $id : 'topic/' . $id;

        $res = $db->q('SELECT author, time FROM ' . ($type == 'topic' ? 'topics' : 'replies') . ' WHERE id = ?', $id);
        ...
        if (isset($_POST['confirm'])) {
            ...
            } elseif ($post->author != $_SESSION['UID']) {
                error::fatal('You are not the author of that post.');
            } elseif ($perm->get('edit_limit') != 0 && ($_SERVER['REQUEST_TIME'] - $post->time > $perm->get('edit_limit'))) {
    ...
    case 'delete_ip_ids':
        ...
        if (isset($_POST['confirm'])) {
            ...
            while ($uid = $res->fetchColumn()) {
                if ($perm->is_admin($uid) || ($perm->is_mod($uid) && !$perm->is_admin())) {
                    error::fatal(m('Error: Access denied'));
                }
            }
