...
if (is_array($_POST['unban'] ?? null) && check_token()) {
...
while ($ban = $res->fetchObject()) {
    ...
    $target = match ($ban->action) {
        'ban_ip' => '<a href="' . DIR . 'IP_address/' . htmlspecialchars($ban->target) . '">' . htmlspecialchars($ban->target) . '</a>',
        'ban_uid' => '<a href="' . DIR . 'profile/' . htmlspecialchars($ban->target) . '">' . htmlspecialchars($ban->target) . '</a>',
        default => htmlspecialchars($ban->target),
    };

    $values = [
        '<input type="checkbox" name="unban[]" value="' . htmlspecialchars($ban->target) . '" class="inline" onclick="highlightRow(this)" />' . $target,
        htmlspecialchars($ban->reason),
        '<a href="' . DIR . 'profile/' . htmlspecialchars($ban->mod_uid) . '">' . ($perm->get_name($ban->mod_uid) ?: htmlspecialchars($ban->mod_uid)) . '</a>',
