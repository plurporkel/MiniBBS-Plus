...
if (isset($_POST['form_sent'])) {
    check_token();
    $ignore_list = $_POST['ignore_list'] ?? '';
    check_length($ignore_list, 'ignore list', 0, 4000);
    
    if (error::valid()) {
        unset($_SESSION['ignored_phrases']);
        $db->q('INSERT INTO ignore_lists (uid, ignored_phrases) VALUES (?, ?) ON DUPLICATE KEY UPDATE ignored_phrases = ?', $_SESSION['UID'], $ignore_list, $ignore_list);
