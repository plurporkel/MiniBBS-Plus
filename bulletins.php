...
if (isset($_POST['bulletin'])) {
    $bulletin = trim($_POST['bulletin']); // Replaced super_trim() with trim()
    ...
    if ($_SESSION['post_count'] < MIN_BULLETIN_POSTS && !$perm->is_admin()) {
        error::add('Sorry, only regulars can post bulletins. You currently have ' . (int)$_SESSION['post_count'] . ' posts, but need ' . (int)MIN_BULLETIN_POSTS . '.');
    }
...
if ($perm->get('bulletin')): ?>
    <form action="" method="post">
        ...
        <div>
            <textarea id="bulletin" name="bulletin" onkeydown="updateCharactersRemaining('bulletin', 'numCharactersLeftForBulletin', 512);" onkeyup="updateCharactersRemaining('bulletin', 'numCharactersLeftForBulletin', 512);" maxlength="512"><?php echo htmlspecialchars($bulletin ?? '') ?></textarea>
        </div>
        <div class="row">
