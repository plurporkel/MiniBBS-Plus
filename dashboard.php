...
if (isset($_POST['form_sent'])) {
    ...
    // Specific validations and transformations
    ...
    foreach ($default_dashboard as $option => $prop) {
        ...
        if (isset($prop['max_length']) && strlen($new_settings[$option]) > $prop['max_length']) {
        ...
        if (isset($prop['type'])) {
            if ($prop['type'] === 'int') {
                if ($new_settings[$option] === '') {
                    $new_settings[$option] = $prop['default'];
                } elseif (!ctype_digit($new_settings[$option]) || strlen($new_settings[$option]) > 25) {
                    error::add('"' . $option . '" does not look like a valid integer.');
                }
            }
            if ($prop['type'] === 'bool' && !in_array($new_settings[$option], ['0', '1'])) {
    ...
    if (error::valid()) {
        $update_count = 0;
        foreach ($default_dashboard as $option => $prop) {
            if ($option === 'memorable_password' && empty($new_settings[$option])) {
                continue;
            }
...
<form action="" method="post">
    ...
    <div class="row">
        ...
        <select id="topics_mode" name="form[topics_mode]" class="inline">
            <option value="0"<?php echo $_SESSION['settings']['topics_mode'] == 0 ? ' selected' : '' ?>>Last post (default)</option>
            <option value="1"<?php echo $_SESSION['settings']['topics_mode'] == 1 ? ' selected' : '' ?>>Date created</option>
        </select>
    ...
    <div class="row">
        ...
        <p class="caption"><strong>Standard options</strong> (click to add):
            ...
            foreach ($template->menu_options as $text => $path) {
                ...
                echo '· <span onclick="document.getElementById(\'custom_menu\').value += \' ' . $text . '\';">' . $text . '</span> ';
            }
            ?>
        </p>
    ...
    <div class="row">
        ...
        <select id="snippet_length" name="form[snippet_length]" class="inline">
            <option value="80"<?php echo $_SESSION['settings']['snippet_length'] == 80 ? ' selected' : '' ?>>80 (default)</option>
            <option value="100"<?php echo $_SESSION['settings']['snippet_length'] == 100 ? ' selected' : '' ?>>100</option>
            <option value="120"<?php echo $_SESSION['settings']['snippet_length'] == 120 ? ' selected' : '' ?>>120</option>
            <option value="140"<?php echo $_SESSION['settings']['snippet_length'] == 140 ? ' selected' : '' ?>>140</option>
            <option value="160"<?php echo $_SESSION['settings']['snippet_length'] == 160 ? ' selected' : '' ?>>160</option>
        </select>
    ...
    <div class="row">
        ...
        <input type="checkbox" id="spoiler_mode" name="form[spoiler_mode]" value="1" class="inline"<?php echo $_SESSION['settings']['spoiler_mode'] ? ' checked="checked"' : '' ?> />
    ...
    <div class="row">
        ...
        <input type="checkbox" id="ostrich_mode" name="form[ostrich_mode]" value="1" class="inline"<?php echo $_SESSION['settings']['ostrich_mode'] ? ' checked="checked"' : '' ?> />
    ...
    <div class="row">
        ...
        <input type="checkbox" id="celebrity_mode" name="form[celebrity_mode]" value="1" class="inline"<?php echo $_SESSION['settings']['celebrity_mode'] ? ' checked="checked"' : '' ?> />
    ...
    <div class="row">
        ...
        <input type="checkbox" id="text_mode" name="form[text_mode]" value="1" class="inline"<?php echo $_SESSION['settings']['text_mode'] ? ' checked="checked"' : '' ?> />
