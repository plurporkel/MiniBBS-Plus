...
// Fetch activity data
$res = $db->q(
    'SELECT activity.action_name, activity.action_id, activity.uid, activity.time, topics.headline 
     FROM activity 
     LEFT OUTER JOIN topics ON activity.action_id = topics.id 
     WHERE activity.time > ? - 960 
     ORDER BY time DESC',
...
while ($row = $res->fetch()) {
    list($action, $action_id, $uid, $age, $headline) = $row;
    ...
    if ($action_text == $old_action) {
        $temp = '<span class="unimportant">(See above)</span>';
    } else {
        $old_action = $action_text;
        $temp = $action_text;
    }
...
// Note if more than 100 users are active
if ($count > 100) {
