...
if (isset($_GET['edit'])) {
    ...
    $res = $db->q('SELECT title, style AS css, name, trip, uid, public, basis, original FROM user_styles WHERE id = ?', $edit_id);
...
if (isset($_POST['form_sent'])) {
    ...
    $style->basis = str_replace(['/', '\\', '.', "\0"], '', $style->basis ?? '');
    $style->color = '';
    ...
    if (isset($_POST['preview']) && check_token()) {
        ...
        /* Flood control */
        $res = $db->q('SELECT COUNT(*) FROM user_styles WHERE uid = ? AND modified > ?', $_SESSION['UID'] ?? '', time() - 300);
        if ($res->fetchColumn() > 9) {
            error::add('You\'re editing too many stylesheets too quickly. Please wait a while.');
        }
        
        /* Uncheck public if duplicate exists */
        ...
        /* XSS protection */
        $dangerous = ['expression', 'binding', 'behavior', 'script', '\\\\', '&', '@import', 'content\s*:'];
        ...
        if (error::valid()) {
            $notice = 'Style saved.';
            if (!($_SESSION['settings']['custom_style'] ?? false)) {
            ...
            if ($editing) {
                $db->q(
                    ...
                    WHERE id = ? AND uid = ?',
                    $style->title, $style->css, $style->md5, $style->public, $style->basis, $style->color, $style->name, $style->trip, time(),
                    $edit_id, $_SESSION['UID'] ?? ''
                );
                ...
                $db->q(
                    ...
                    (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                    $style->title, $style->css, $style->md5, $style->public, $style->basis, $style->color, $style->name, $style->trip, time(), $style->original, $_SESSION['UID'] ?? ''
                );
...
if (!empty($style->title ?? '')) {
    $template->title .= ' (' . htmlspecialchars($style->title, ENT_QUOTES, 'UTF-8') . ')';
}
...
<?php if ($editing && $style->public): ?>
<p>To show others your style, give them this link: <strong><a href="<?php echo DIR . 'view_style/' . (int) $edit_id; ?>"><?php echo URL . 'view_style/' . (int) $edit_id; ?></a></strong></p>
<?php endif; ?>
...
<form action="" method="post">
    ...
    <div class="row">
        ...
        <input id="style_title" name="style[title]" tabindex="1" type="text" size="50" maxlength="60" value="<?php echo htmlspecialchars($style->title ?? '', ENT_QUOTES, 'UTF-8'); ?>">
    </div>
    <div class="row">
        <label for="name">Your name</label>
        <input id="name" name="name" type="text" size="30" maxlength="30" tabindex="2" value="<?php echo htmlspecialchars($set_name, ENT_QUOTES, 'UTF-8'); ?>" placeholder="name #tripcode" />
    </div>
    <div>
        <textarea id="custom_style" name="style[css]" cols="80" rows="29"><?php echo htmlspecialchars($style->css ?? '', ENT_QUOTES, 'UTF-8'); ?></textarea>
    </div>
    <div class="row">
        ...
        <select id="style_basis" name="style[basis]" class="inline">
            ...
            <?php foreach ($base_styles as $style_title): ?>
                <option value="<?php echo htmlspecialchars($style_title, ENT_QUOTES, 'UTF-8'); ?>"<?php if (($style->basis ?? '') == $style_title) echo ' selected'; ?>><?php echo htmlspecialchars($style_title, ENT_QUOTES, 'UTF-8'); ?></option>
            <?php endforeach; ?>
        </select>
    </div>
    <div class="row">
        <input id="style_public" name="style[public]" type="checkbox" value="1"<?php if ($style->public ?? false) echo ' checked="checked"'; ?> class="inline" />
