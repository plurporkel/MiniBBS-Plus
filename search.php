...
if (!empty($search_query)) {
    // Rate limiting: Check search frequency
    $res = $db->q('SELECT COUNT(*) FROM search_log WHERE ip_address = ? AND time > (? - 60)', $_SERVER['REMOTE_ADDR'], time());
    ...
    if (error::valid()) {
        $db->q('INSERT INTO search_log (ip_address, time) VALUES (?, ?)', $_SERVER['REMOTE_ADDR'], time());
        ...
        $topic_results = $db->q(
            'SELECT id, time, replies, visits, poll, locked, sticky, namefag, tripfag, headline, body 
             ...
             ORDER BY time DESC
             LIMIT ?, ?',
            $query_sql, $query_sql, $page->offset, $page->limit
        );
        ...
        $reply_results = $db->q(
            'SELECT replies.id, replies.parent_id, replies.time, replies.body, replies.namefag, replies.tripfag, topics.headline
             ...
             LIMIT ?, ?',
            $query_sql, $page->offset, $page->limit
        );
    }
...
<form action="" method="post">
    <div class="row">
        <input id="phrase" name="phrase" type="text" size="80" maxlength="255" value="<?php echo htmlspecialchars($search_query); ?>" class="inline" autofocus required />
...
if (isset($topic_results)) {
    ...
    while ($reply = $reply_results->fetchObject()) {
        ...
        $headline = '<a href="' . htmlspecialchars(DIR . 'topic/' . $reply->parent_id) . '">' . $headline . '</a>';
        if ($query_pos !== false) {
            $snippet = preg_replace('/(' . $search_query_p . ')/i', '<em class="marked">$1</em>', $snippet);
        }
        $snippet = '<a href="' . htmlspecialchars(DIR . 'topic/' . $reply->parent_id . '#reply_' . $reply->id) . '">' . $snippet . '</a>';
...
$page->navigation('search/' . urlencode($search_query), max($topic_count ?? 0, $reply_count ?? 0));
